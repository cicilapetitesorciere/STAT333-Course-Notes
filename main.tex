...
\begin{example}{DeMontfort's Revistied}
...
\subsection{The Geometric Distribution}
    We once again consider an infinite sequence of Bernoulli trials $I_1,I_2,...\simiid \text{Ber}(p)$. This time we let $X$ be the waiting time for the first success. For instance if $I_1$ returned $1$ then $X$ returns 1. If $I_1$ returns 0 and $I_2$ returns 1 then $X$ returns 2, etc. Or more generally $I_x=1$ and $I_1=I_2=\cdots =I_{x-1}=0$, then $X=x$. We say
    \[
        X\sim \text{Geom}(p)
    \]
    ...
    \begin{theorem}{Properties of the Geometric Distribution}
        ...
        \begin{enumerate}
            \item $\forall x\in\{1,2,...\}, P(X=x)=(1-p)^{x-1}p$
            \item $\E(X)=\nicefrac 1 p$
...
We can generalize the geometric distriution, and let $X$ be the number of Bernoulli trials before we get $k$ successes. Then we say $X$ has a \textbf{Negative Binomial Distribution} or $X\sim \text{NB}(p,k)$.
...
\begin{tabular}{|c|c|c|c|c|c|c|}
     Distribution & Support & PMF & CDF & Expectation & Variances \\
     \hline
\end{tabular}
...
\begin{example}{Denisty Bigger than 1}
